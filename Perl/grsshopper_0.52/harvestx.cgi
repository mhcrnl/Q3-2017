...
$Site->{diag_level} = 9;					# Diagnostics level 0-10
								# 1 - save item
								# 2 - save media, author
								# 3 - save link
								# 6 - scraper
...
for ($action) {					# There is always an action
	...
	/harvest/ && do { &harvest_feed($vars->{id}); last; 		};
...
#	my () = @_;
	...
	my $qtime = time;
					...
					# Find next in queue
	my $stmt = "SELECT feed_id FROM feed WHERE feed_status = 'A' ORDER BY feed_lastharvest LIMIT 0,1";
...
sub harvest_process_data {
	...
	if ($feedrecord->{feedstring} =~ /^BEGIN:VCALENDAR/) {
		...
		return;	
	} elsif ($feedrecord->{feedstring} =~ /^(<\?xml|<rss)/i) {
...
sub get_url {
	...
	my $cache = &feed_cache_filename($feedrecord->{feed_link});
	...
	if ((time - (stat($cache))[9]) < (60*60)) {			# If the file is less than 1 hour old
		...
		unless ($feedrecord->{feedstring}) {
			...
			return;
									...
									# Save common cache
		open FOUT,">$cache" or die "Error opening to write to $cache: $!";
		print FOUT $feedrecord->{feedstring}  or die "Error writing to $cache: $!";
		close FOUT;
		chmod 0666, $cache or die "Couldn't chmod $cache: $!";
	}
...
sub feed_cache_filename  {
	...
	return $Site->{feed_cache_dir}.$feed_file;
}
...
sub parse_feed {
	...
	my ($feedrecord) = @_;
	&diag(8,"<form><textarea cols=140 rows=80>".$feedrecord->{feedstring}."</textarea><form>");	
	...
	last if ($linecount > 500);
		...
		if ($vars->{content} eq "on") {				# If we're processing content
			...
			unless ($tag =~ /(^\/content|^\/description$)/) {	#   then unless we're closing the content tag
...
#	my $parenttype = @{$feed->{objectstack}}[1]->{type};
	...
	if ($type eq "link") {
		...
		&diag(9,"link ".$child->{attributes}."<br>");
		...
		&diag(4," - - <b>AUTHOR</b> Saving $child->{type}, to $parent->{type}<p>");
	}	
	...
	if ($tag =~ /(^content$|^content:encoded$|^description$)/i) {  $vars->{content} = "off";  }
	...
	if ($type) { shift @{$feed->{objectstack}}; }	# If object, remove object
...
sub save_records {
	...
	foreach my $item (@{$feed->{items}}) {
		...
		if (&is_existing_link($item)) { &diag(1," -- Already Exists<br>"); next; }
		&diag(1," -- New item <br>");
...
sub is_author {
	...
	unless ($author->{author_name} || $author->{author_email} || $author->{author_link} || $author->{author_id}) { 
		&diag(9,"<p>Author from $author->{source} rejected; it has no name, email, url or id</p>"); return 0; }
	if ($author->{author_name} =~ /^admin$/i) { 
		&diag(9,"<p>Author from $author->{source} rejected; 'admin' is not an author name</p>"); return 0; }	
	if ($author->{author_name} =~ /^guest$/i) { 
		&diag(9,"<p>Author from $author->{source} rejected; 'guest' is not an author name</p>"); return 0; }		
	return 1;
...
sub is_existing_link {
	...
	if ($item->{link_id}) { 
		...
		if ($tl->{link_orig} eq "yes") { &diag(1,"Existing item.<br>"); return  1; } 
	} 
	return 0;
}
