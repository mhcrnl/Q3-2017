...
#	print "User are go<p>";
	...
	&db_update($dbh,"person",{person_mode => $sessionid}, $Person->{person_id});
...
#	print "Done cookies<p>";
	...
	if ($Person->{person_id} && $Person->{person_id} ne "2") {
		...
		if ($target) {		# Site-specific Need a better thing here
			if ($target =~ /edfuture/) {			# URL-specific - Need a better thing here
				...
				print qq|Return to D2L website - <a href="$redirect">Click here</a><p>|;exit;
...
sub print_nav_options {
	...
	print qq|<li><a href="$script?action=Options$refera">
		Edit Your Personal Information and Options</a></li>|;
		
	unless ($Person->{person_openid}) {
		unless ($Person->{person_id} eq 2) {
			if ($vars->{openid_enabled}) {
				print qq|<li><a href="$script?referq&action=openidloginform">
	...
	unless ($Person->{person_id} eq 2) {
		...
		print qq|<li><a href="$script?action=Logout$refera">Logout</a></li>|;
	}
	print "</ul></p>";
