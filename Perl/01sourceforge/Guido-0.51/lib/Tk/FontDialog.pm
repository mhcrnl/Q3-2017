...
#
# $Id: FontDialog.pm,v 1.2 2001/04/05 01:39:47 jtillman Exp $
# Author: Slaven Rezic
#
...
#	 fontActual without an argument doesn't work on 
#     Win32, it seems, but cget(-font) does
	my $dialog_font;
	if ($^O ne "MSWin32") {
		eval {
			$dialog_font = $w->fontConfigure($w->fontActual
			     ($w->optionGet("font", "*")));
				};
		if ($@) {
			$dialog_font = $w->fontCreate($w->fontActual
			     ($w->optionGet("font", "*")));
		}
	}
	else {
	    $dialog_font = $w->fontCreate($w->cget(-font),
					     ($w->optionGet("font", "*")));
	}
...
sub Show {
    ...
    #On Win32, using InsertFamilies here doesn't update the HList
    # Placing another call to it after the focus() below seems to
    # work, for some reason.  If this first call is commented out,
    # however, the second fails, too.  Very strange.
    $w->InsertFamilies();
    $w->UpdateFont();
    # XXX ugly...
    ...
    #$w->waitVisibility;
    $w->focus;
    #This call was added to make it work on Win32
    $w->InsertFamilies() if $^O eq 'MSWin32';
