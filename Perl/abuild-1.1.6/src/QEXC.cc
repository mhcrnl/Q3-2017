...
QEXC::TimeOut::TimeOut(std::string const& prefix)
{
    this->setMessage(prefix + ": timed out");
}

int
QEXC::errno_wrapper(std::string const& description, int status)
    throw (QEXC::System)
{
    if (status == -1)
    {
	throw System(description, errno);
    }
    return status;
}

FILE*
QEXC::fopen_wrapper(std::string const& description, FILE* f)
    throw (QEXC::System)
{
    if (f == 0)
    {
