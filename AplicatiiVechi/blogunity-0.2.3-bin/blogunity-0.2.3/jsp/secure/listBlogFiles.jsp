...
<div id="contentLayer">
	<div class="title"><b:i18n key="LIST_FILES_TITLE" /></div>
	<div class="description">
		<b:i18n key="LIST_FILES_DESC" />
	</div>


	<div class="smalltitle"><b:i18n key="UPLOAD_FILE" /></div>
	<div id="formLayer">
		<form method="post" action="<%=ctx%>/uploadFile.secureaction" enctype="multipart/form-data">
			<%if (errors!=null && errors.containsErrorsForKey("resource")){ %>
				<div class="textError" onclick="toggle('errorLayerResource')"><b:i18n key="FILE_NAME"/>:</div> 
			<%}else{ %>
				<div class="textRequired"><b:i18n key="FILE_NAME"/>:</div> 
			<%}%>
			...
			<%}%>
			<input type="submit" name="upload image" value="<b:i18n key="UPLOAD_FILE_BUTTON" />" />
			<br/>
	...
	<div id="formLayer">
		<form method="post" action="<%=ctx%>/createBlogFilesDirectory.secureaction">
	...
	<br/>
	/ <a href="<%=ctx%>/listBlogFiles.secureaction?id=<%=b.getId()%>">files</a>	
	<%if (org.apache.commons.lang.StringUtils.isNotEmpty(path)){
        if (path.charAt(0) == '/') path = path.substring(1);
		String[] splitted = path.split("/");
        String url = ctx + "/listBlogFiles.secureaction?id="  + b.getId();
	...
	<display:table name="files" decorator="com.j2biz.blogunity.web.decorator.FilesTableDecorator" 
				requestURI="listBlogFiles.secureaction" pagesize="20" defaultsort="1" defaultorder="descending" sort="list">
		...
		<display:column property="filesActions" titleKey="ACTIONS" />
	</display:table>
